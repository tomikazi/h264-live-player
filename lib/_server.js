...
class _Server {
    ...
    new_client(socket) {
        ...
        socket.on("message", function (data) {
            if (data[0] === '{') {
                ...
                if (action === "REQUESTSTREAM") {
                    self.client.publish('LightBar/power', 'on');
                    self.start_feed();
                }
        ...
        socket.on('close', function () {
            ...
            self.client.publish('LightBar/power', 'off');
    ...
    _on_connect() {
        ...
        this.client.subscribe([pfx + '/status', pfx + '/view', pfx + '/location', pfx + '/track', pfx + '/track/status', pfx + '/station/status']);
    }
    ...
    process_command(d) {
        if (d.station) {
            ...
            this.client.publish(pfx + '/track', d.track.toLowerCase());
        } else if (d.view) {
