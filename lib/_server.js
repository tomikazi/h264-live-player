...
class _Server {
    ...
    processPanAndTilt(d) {
        var pv = Math.round(Math.abs(d.pan)),
            tv = Math.round(Math.abs(d.tilt)/16);

        if (pv && !panning) {
            panning = true;
            pan.direction = d.pan > 0;
            pdir = pan.direction ? "left" : "right";
            pan.turn(pv).then(steps => { console.log(`Panned ${steps} steps ${pdir}`); panning = false;} );
        }
        ...
        if (tv && !tilting) {
            ...
            tilt.turn(tv).then(steps => { console.log(`Tilted ${steps} steps ${tdir}`); tilting = false;} );
        }
    }
