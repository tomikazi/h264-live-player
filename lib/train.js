...
class TrainTracker {
    ...
    track() {
        if (this.location === 'Forest' && this.nextTrack !== 'unknown') {
            ...
            setTimeout(e => { if (this.running) { this.move_to(this.nextTrack, 6) }}, 500);
        } else if (this.location === 'Tunnel') {
            // Wait a bit and then start tracking view to the forest location; slowly
            setTimeout(e => this.move_to('Forest', 10), 500);
        } else {
            // Wait a bit and then start tracking view to the tunnel location; slowly
            setTimeout(e => { if (this.running) { this.move_to('Tunnel', 10) }}, 1000);
        }
    }
